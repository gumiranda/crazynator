...
/**
 * Function to handle batch sandbox recreation for multiple fragments
 */
export const batchRecreateSandboxesFunction = inngest.createFunction(
  {
    id: 'batch-recreate-sandboxes',
    concurrency: 1, // Process one batch at a time
  },
  { event: 'sandbox/batch-recreate' },
  async ({ event, step }) => {
    const { fragmentIds } = event.data;

    console.log(`Starting batch recreation for ${fragmentIds.length} fragments`);

    const results = await step.run('batch-recreate', async () => {
      const { prisma } = await import('@/lib/prisma');
      const { recreateMultipleSandboxes } = await import('@/lib/sandbox-recreation');
      ...
      if (fragments.length === 0) {
        return { successful: [], failed: [], summary: { total: 0, successful: 0, failed: 0 } };
      }

      // Recreate all fragments
      return await recreateMultipleSandboxes(fragments);
    });

    console.log(
      `Batch recreation completed: ${results.summary.successful}/${results.summary.total} successful`,
    );
